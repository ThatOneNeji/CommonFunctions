...
class MessageBroker {
    ...
    init(options = {}) {
        ...
        Connection.on('connect', function (connectionn) {
            Logging.info('Connected to: ' + connectionn.url);
        ...
        Connection.on('connectFailed', function (connect) {
            Logging.error(connect);
        });
        Connection.on('blocked', function (reason) {
            Logging.warning(reason);
        });
        Connection.on('unblocked', function (unblocked) {
    ...
    createPublishQueue(queueName) {
        if (this.publishChannelWrapper) {
            ...
            self.initialisePublishQueue(queueName);
        }
    }
    ...
    batchPublish(payloads) {
        ...
        while (payloads.length > 0) {
            batches.push(payloads.splice(0, 1000));
        }
        for (const batch in batches) {
            if (Object.hasOwn(batches, batch)) {
                const elementes = batches[batch];
                for (const msg in elementes) {
                    if (Object.hasOwn(elementes, msg)) {
                        const element = elementes[msg];
                        if (element.data) {
